...
// ADR-0004 P6: `deal build` enforces import visibility (strict closure). Golden
// fixtures build standalone (no workspace/stdlib), so the fixture now declares its
// port payload type (`attribute def Power`) locally instead of relying on an
// unimported `deal.std.units.Power` — keeping it import-clean in isolation.
#[test]
